...
A machine learning service that predicts which stocks will outperform the S&P 500 index over a 12-moth horizon. It is served through a FastAPI REST API with a Streamlit dashboard client.
...
6. Front end displayed the prediction as a probability and a binary "Predicted to Beat S&P 500" flag
...
6. Start the frontned: `cd frontend && uv run streamlit run main.py`
...
**Response(200):** `ticker`, `outperfomance_probability` (0.0-1.0), and `predicted_class` (1 if `outperfomance_probability` > 0.5, else 0)
...
**Target variables:** Binary. A stock is labeled as `1` if its trailing 12-month return exceeded the S&P 500's trailing 12-month return over the same perdiod, `0` otherwise.
...
- **No features importance analysis:** The model is a black box. There's no SHAP or permutation importance to explain twhich ratios are driving predictions for a given stock
...
- *Summary endpoint* returning feature contributions alonside the prediction
- *Broader universe of stocks* like the Russel 1000 or all NYSE/NASDAQ stocks to increase training set size and reduce survivorship bias

## Design decisions

- **Binary target over regression:** Predicting if it will outperfroem rather than predicting by how much since for most retail investors the decision is either buy or don't
- **12-moth return window:** Convention in financial research; short enough to be actionable but long enough to smooth out noise
...
- **Median imputation:** Some tickers are missing certain fundamentals; Median imputation is simple and avoids data leakage since it's computer per-fold inside the pipeline
...
Mocking `yf.Fianace` at the class level rather than intercepting HTTP calls gave stable, readable integration tests.

### Survivorship bias is easy to ignore and fard to fix

Achnowledging it honestly is better than pretending it doesn't exist. A v2 with historical constituent lists would address it properly, but that might require a paid data source.
